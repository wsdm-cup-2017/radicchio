...
for i in range(len(train['Person'])):
    wiki = wikipedia.page(train['Person'][i])
...
import os
cwd = os.getcwd()
train.to_csv('pr_wiki.txt',columns=('Person','Profession','Score','wiki'),sep="\t")
...
df['Score']=train['Score']
df.to_csv('features_person.txt',sep="\t")
